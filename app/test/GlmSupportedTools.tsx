...
export default function GlmSupportedTools({ posts, settings }: { posts: PostSummary[]; settings?: SiteSettings }) {
  ...
  return (
    <section className="relative w-full overflow-hidden px-5 py-16 sm:px-8">
      <div className="mx-auto max-w-6xl">
        ...
        <GlmReveal stagger className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {tools.map((tool, index) => {
            ...
            return (
              <Link
                ...
                className={`group flex h-full flex-col ${glassCard} p-6 transition hover:scale-[1.02] hover:border-primary-400/60 hover:shadow-2xl dark:hover:border-primary-400/50`}
              >
                <div className={`h-1 rounded-full ${accentBars[index % 4]}`} />
                <div className="mt-5 flex items-center justify-between gap-2">
                  {!content.hidePromptCounts ? (
                    <span className="inline-flex items-center gap-1 rounded-full border border-white/60 bg-white/40 px-2.5 py-1 text-[11px] font-bold text-surface-600 dark:border-white/10 dark:bg-white/5 dark:text-surface-300">
                ...
                <div className="mt-5 grid grid-cols-3 gap-1 rounded-2xl border border-white/60 bg-white/40 p-3 dark:border-white/10 dark:bg-white/5">
