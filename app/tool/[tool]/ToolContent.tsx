        ...
        <div className="grid gap-8 p-6 sm:p-8 lg:grid-cols-[1fr_auto] lg:items-center lg:p-10">
          <div>
            <div className="mb-5 flex h-16 w-16 items-center justify-center overflow-hidden rounded-2xl bg-primary-100 text-primary-500 dark:bg-primary-900/30">
              {toolInfo.logo ? (
                <span className="relative h-11 w-11 rounded-full bg-white shadow-sm" style={toolInfo.logoScale ? { transform: `scale(${toolInfo.logoScale})` } : undefined}>
                  <Image src={toolInfo.logo} alt={`${tool} logo`} fill sizes="44px" className="rounded-full object-cover" referrerPolicy="no-referrer" />
                </span>
              ) : (
            ...
            <p className="max-w-2xl text-lg leading-8 text-surface-600 dark:text-surface-300">
              Discover {filtered.length} curated prompt {filtered.length === 1 ? 'collection' : 'collections'} prepared for {tool}.
            </p>
          </div>
