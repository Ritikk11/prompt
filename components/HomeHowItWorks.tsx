...
const checkRow =
  'flex items-center gap-3 rounded-xl border border-white/60 bg-white/25 px-4 py-3 text-sm font-medium text-surface-700 dark:border-white/10 dark:bg-white/5 dark:text-surface-200';
            ...
            {editableSteps.map((step, index) => {
              ...
              return (
                <button
                  ...
                  className={`group grid w-full grid-cols-[auto_1fr_auto] items-start gap-4 rounded-2xl border p-5 text-left backdrop-blur-md backdrop-saturate-150 transition ${isActive
                      ? 'border-primary-500/70 bg-primary-500/10 shadow-[0_16px_44px_rgba(26,115,232,0.16)]'
                      : 'border-white/60 bg-white/25 hover:border-primary-400/60 hover:bg-white/60 dark:border-white/10 dark:bg-white/[0.04] dark:hover:border-primary-400/50 dark:hover:bg-white/[0.08]'
                    }`}
                >
                  <span className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl ${step.color} text-sm font-black shadow-lg`}>
                    {step.number}
