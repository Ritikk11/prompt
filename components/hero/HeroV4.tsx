    ...
    <div className="relative w-full rounded-2xl overflow-hidden">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-2">
        ...
        <Link href={`/${topFeatured[0].slug || topFeatured[0].id}`} className="relative h-[400px] lg:h-[500px] lg:col-span-2 rounded-2xl overflow-hidden group">
          ...
          <LoadingImage src={promptImageUrl(topFeatured[0])} alt={topFeatured[0].title} fill priority sizes="(max-width: 1024px) 100vw, 66vw" showSkeleton={showSkeleton} className="object-contain transition-transform duration-700 group-hover:scale-105"  referrerPolicy="no-referrer" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-2">
          {topFeatured.slice(1).map((fPost) => (
            <Link key={fPost.id} href={`/${fPost.slug || fPost.id}`} className="relative h-[200px] sm:h-[250px] lg:h-[calc(250px-4px)] rounded-2xl overflow-hidden group">
              ...
              <LoadingImage src={promptImageUrl(fPost)} alt={fPost.title} fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" showSkeleton={showSkeleton} className="object-contain transition-transform duration-700 group-hover:scale-105"  referrerPolicy="no-referrer" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
